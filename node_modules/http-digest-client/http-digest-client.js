...
var HTTPDigest = function () {
  ...
  HTTPDigest.prototype.request = function (options, callback) {
    ...
    var req = http.request(options, function (res) {
      self._handleResponse(options, req, res, callback);
    });
    req.__write = req.write
    req.write = function (data, encoding, callback) {
      req.__write(data, encoding, callback);
      self._bufferedWrites.push({data: data, encoding: encoding});
    };
    return req;
  };
  ...
  //
  HTTPDigest.prototype._handleResponse = function handleResponse(options, clientRequest, res, callback) {
    ...
    if (bufferedWrites.length === 0) {
      ...
      (function writeNext() {
        if (i < bufferedWrites.length) {
          ...
          req.write(bufferedWrite.data, bufferedWrite.encoding, writeNext);
        } else {
          req.end();
        }
      })();
  ...
  HTTPDigest.prototype._parseChallenge = function parseChallenge(digest) {
    var authVerb = digest && digest.split(' ')[0].toLowerCase();
    var challenge = digest.substr(digest.indexOf(' '));
